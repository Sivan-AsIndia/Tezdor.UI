...
export class DayProductionCreateComponent {
  ...
  pageTitle = computed(() => this.editMode() ? 'Production Entry' : 'Production Entry');
...
constructor() {
  this.productionForm.get('productionNo')?.setValue(this.service.generateProductionNo());
  this.productionForm.get('productionDate')?.setValue(new Date().toISOString().split('T')[0]);
  ...
  onWorkOrderChange(workOrderId: string): void {
    ...
    if (!workOrder) { this.consumptionRows.set([]); return; }

    const bomLines = WORK_ORDER_BOM_SEED.filter((line) => line.workOrderId === workOrderId);

    if (bomLines.length === 0) { this.consumptionRows.set([]); return; }
    ...
    const rows: ConsumptionRow[] = bomLines.map((line, index) => {
      ...
      const materialCode =
        ...
        (line.rawMaterialId ? resolveMaterialCode(this.products, line.rawMaterialId) : '');
  ...
  getVariance(row: ConsumptionRow): number {
    return row.plannedQty !== null ? row.actualQty - row.plannedQty : row.actualQty;
  }
  ...
  // ── Build payload from form + selected WO ──
